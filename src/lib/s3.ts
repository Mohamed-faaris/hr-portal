...
export async function getAllResumesInJobFolder(jobId: string, expiresIn: number = 3600): Promise<Array<{ filename: string; url: string }>> {
    try {
        ...
        return presignedUrls.filter((url) => url !== null) as Array<{ filename: string; url: string }>;
    } catch (error) {
        return [];
    }
}
